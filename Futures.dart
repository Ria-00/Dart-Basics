...
Future<String> fetchDataWithTimeout(int i){
  if (i<5) {
    return Future.delayed(Duration(seconds:i),()=> Future.value('Data fetched successfully!'));
  } else {
    return Future.delayed(Duration(seconds: 5),()=> throw TimeoutException('TimeOut!'));
  }
}
...
Future<Map<String, dynamic>> fetchUserData(int id) => Future.delayed(
    Duration(seconds: 3),
    () => Future.value(
        {"name": "Rio", "age": 20, "email": "rio.singh@gmail.com"}));
...
for(int i=0;i<3;i++)
     Future.delayed(
        Duration(seconds: 2), () => Future.value("Success : $url"));
 {
